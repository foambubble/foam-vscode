...
export function isFileTooLarge(document: TextDocument): boolean {
  const sizeLimit = workspace
    .getConfiguration("markdown.extension.syntax")
    .get<number>("decorationFileSizeLimit");
...
export function showChangelog() {
  commands.executeCommand(
    "vscode.open",
    Uri.parse(
      "https://github.com/yzhang-gh/vscode-markdown/blob/master/CHANGELOG.md"
    )
  );
}
...
export function slugify(heading: string, mode?: string, downcase?: boolean) {
  if (mode === undefined) {
    mode = workspace
      .getConfiguration("markdown.extension.toc")
      .get<string>("slugifyMode");
  }
  if (downcase === undefined) {
    downcase = workspace
      .getConfiguration("markdown.extension.toc")
      .get<boolean>("downcaseLink");
  }
